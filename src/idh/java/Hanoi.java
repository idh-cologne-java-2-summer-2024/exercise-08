...
public class Hanoi {
	...
	private void movePieces(int numberOfPieces, char from, char to, char util) {
		// TODO: Implement me!
		numberOfPieces = 7;//unsure if numberOfPieces defined elsewhere?
		Integer piece = left.remove();
		if(!left.isEmpty() && middle.isEmpty() && right.isEmpty()) { //starting positions
			while(left.descendingIterator().hasNext()) { //iterate over starting set 
				Integer compVal = piece.intValue();
				switch(compVal) {
				case 3:
					right.offer(left.poll());
				case 4:
					middle.offer(left.poll());
				case 5:
				case 6:
				case 7:
				case 8:
...
//			Integer piece2 = left.remove();
//			//move disk from m to r etc...
//			middle.add(piece2);
//			right.remove();
//			middle.add(piece);
		}
