...
public class Hanoi {
     ...
     * from and to are `char` values l, m, or r for the left, middle, or right
    ...
    private void movePieces(int numberOfPieces, char from, char to, char util) {
        if (numberOfPieces == 1) {
            // Basisfall: Verschieben Sie eine Scheibe direkt von `from` nach `to`.
            movePiece(from, to);
            System.out.println(this); // Ausgabe des aktuellen Zustands der Türme
        } else {
            // Rekursionsschritt:
            // a) Verschieben Sie n-1 Scheiben von `from` nach `util`, wobei `to` als Hilfsstab verwendet wird.
            movePieces(numberOfPieces - 1, from, util, to);

            // b) Verschieben Sie die `n`-te Scheibe von `from` nach `to`.
            movePiece(from, to);
            System.out.println(this); // Ausgabe des aktuellen Zustands der Türme

            // c) Verschieben Sie n-1 Scheiben von `util` nach `to`, wobei `from` als Hilfsstab verwendet wird.
